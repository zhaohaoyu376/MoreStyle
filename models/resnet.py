...
class ResNet(nn.Module):
    def __init__(self, block, layers, ini=1, num_classes=1000,mixstyle_layers=[],random_type=None,p=0.5):
        self.inplanes = 64
        ...
        if mixstyle_layers:
            ...
            elif random_type == 'DSU':
                self.random = DSU(p=p, mix='random')
            else:
    ...
    def forward(self, x, mix=True):
        ...
        x = self.bn1(x)
        x = self.relu(x)
        if 'layer0' in self.mixstyle_layers and mix:
            x = self.random(x)
        sfs.append(x)
        x = self.maxpool(x)
        x = self.layer1(x)
        if 'layer1' in self.mixstyle_layers and mix:
            x = self.random(x)
        sfs.append(x)
        x = self.layer2(x)
        if 'layer2' in self.mixstyle_layers and mix:
            x = self.random(x)
        sfs.append(x)
        x = self.layer3(x)
        if 'layer3' in self.mixstyle_layers and mix:
            x = self.random(x)
        sfs.append(x)
        x = self.layer4(x)
        if 'layer4' in self.mixstyle_layers and mix:
            x = self.random(x)
        sfs.append(x)
...
def resnet34(ini=3,pretrained=False, **kwargs):
    """Constructs a ResNet-34 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(BasicBlock, [3, 4, 6, 3],ini=ini, **kwargs)
    if pretrained:
